...
def num_translate(word):# word - параметр.
    if word.istitle(): # если наше слово начинается с первой большой буквы. То, наш фильтр должен реагировать на это.
        return str(translation_dict.get(word.lower())).title()
        #наш фильтр. Мы обращамся к словарю, затем ставим get(чтобы не было ошибки, а было просто none).
        # Преобразуем так, чтобы символы сводились к одному. Чтобы слова(word),если они в верхнем регистре,
        # то входили сначала всеми буквами в нижнем регистре. -  .lower
        # А затем выпускались так, чтобы первая заглавная буква была в верхнем.  - .title.
        # Чтобы не было ошибки, мы сначала приобразуем всё к строке.
    return translation_dict.get(word)
print(num_translate(input("Write number: ")))